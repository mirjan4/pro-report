...
import {
  Layers, Plus, Edit, Trash2, Sparkles, Check, X,
  Upload, Download, AlertTriangle, CheckCircle,
  FileSpreadsheet, Trash, Package, ChevronDown
} from 'lucide-react';
...
const CollectionEntry = () => {
  const { financialYears, selectedFY, pros, selectedModule, modules, refreshMetadata } = useApp();
  ...
  const modulePros = selectedModule
    ? pros.filter(p => {
  ...
  const fetchCollections = async () => {
    ...
    try {
      ...
      if (selectedModule) params.append('module', selectedModule._id);
  ...
  useEffect(() => {
    if (financialYears.length > 0) {
      const active = selectedFY || financialYears.find(fy => fy.isActive) || financialYears[0];
      setFyId(active._id);
    }
    if (activePros.length > 0) setProId(activePros[0]._id);
  }, [financialYears, selectedFY, selectedModule, pros]);

  const openAddForm = (defaultMonth) => {
    setEditingEntry(null);
    setMonth(defaultMonth || 'April');
    if (activePros.length > 0) setProId(activePros[0]._id);
    setAmount('');
    setNotes('');
    setError('');
  ...
  const openEditForm = (entry) => {
    ...
    setFyId(entry.financialYear?._id || entry.financialYear);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!fyId || !month || !proId) {
      setError('Please select financial year, month, and PRO');
      return;
    }
    if (!selectedModule) {
  ...
  const downloadSampleTemplate = () => {
    const headers = ['Financial Year', 'Month', 'PRO Name', 'Amount', 'Notes'];
    const sampleRows = [
      ['2024-25', 'April', 'Ahmed Al-Rashidi', '75000', ''],
      ['2024-25', 'April', 'Fatima Malik', '60000', 'On time']
    ];
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        const requiredFields = ['Financial Year', 'Month', 'PRO Name', 'Amount'];
        ...
        const validated = rows.map((row, idx) => {
          const errors = [];
          const rawFY = row['Financial Year']?.toString().trim();
          const rawMonth = row['Month']?.toString().trim();
          const rawPro = row['PRO Name']?.toString().trim();
          const amt = Number(row['Amount']) || 0;
          const rowNotes = row['Notes']?.toString().trim() || '';

          const resolvedFY = financialYears.find(fy => fy.year === rawFY);
          if (!resolvedFY) errors.push(`FY "${rawFY}" not found`);
          if (!MONTHS.includes(rawMonth)) errors.push(`Invalid month: "${rawMonth}"`);

          // Match PRO against module-filtered list
          ...
          const isDuplicate = resolvedFY && resolvedPRO && MONTHS.includes(rawMonth)
            ? collections.some(c => {
                const cFyId = c.financialYear?._id || c.financialYear;
                const cProId = c.pro?._id || c.pro;
                return cFyId === resolvedFY._id && c.month === rawMonth && cProId === resolvedPRO._id;
              })
            : false;

          return {
            rowNum: idx + 2,
            fyYear: rawFY, fyId: resolvedFY?._id,
            month: rawMonth, proName: rawPro, proId: resolvedPRO?._id,
            amount: amt, notes: rowNotes,
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <p className="text-gray-400 text-sm">Submit and manage monthly collection records</p>
          </div>
        </div>
        {activeTab === 'registry' && (
          <button
            onClick={openAddForm}
            className="flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 glow-btn text-dark-bg"
            style={{ background: `linear-gradient(135deg, ${moduleColor}, ${moduleColor}cc)` }}
          >
            <Plus className="w-4 h-4" />
            <span>New Monthly Entry</span>
          </button>
        )}
      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  {MONTHS.map(m => {
                    ...
                    const monthTotal = monthEntries.reduce((sum, c) => sum + (c.amount || 0), 0);
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                      {monthEntries.map(c => (
                                        <tr key={c._id} className={`hover:bg-white/[0.01] transition-colors text-gray-300 ${selectedCollectionIds.includes(c._id) ? 'bg-gold/[0.02]' : ''}`}>
                                          ...
                                          <td className="px-5 py-3.5 text-gray-400">{c.pro?.area || '—'}</td>
                ...
                <div className="glass-card rounded-2xl p-5 border border-white/10 bg-white/[0.02] flex items-center justify-between gap-4 mt-6">
                  ...
                  <div className="text-right">
                    <span className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider block">Grand Total</span>
                    <span className="text-2xl font-extrabold" style={{ color: moduleColor }}>
                      ₹{collections.reduce((s, c) => s + (c.amount || 0), 0).toLocaleString('en-IN')}
                    </span>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-gray-400">Template columns: Financial Year, Month, PRO Name, Amount, Notes</p>
            </div>
            <button onClick={downloadSampleTemplate}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 text-sm font-semibold text-gray-200 hover:bg-white/5 hover:border-white/20 transition-all">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-11 h-6 rounded-full transition-colors duration-200 flex items-center px-1 ${skipDuplicates ? '' : 'bg-white/10'}`}
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-white/5 text-gray-400">
                      {['Row', 'FY', 'Month', 'PRO Name', 'Amount', 'Status', 'Issues'].map(h => (
                  ...
                  <tbody>
                    {parsedRows.map(row => (
                      <tr key={row.rowNum} className="border-b border-white/5">
                        ...
                        <td className="px-3 py-2 text-gray-300">{row.fyYear}</td>
                        <td className="px-3 py-2 text-gray-300">{row.month}</td>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-dark-bg disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  style={{ background: `linear-gradient(135deg, ${moduleColor}, ${moduleColor}cc)` }}
                >
                ...
                <button onClick={clearBulkState} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 text-sm font-semibold text-gray-400 hover:text-white hover:bg-white/5 transition-all">
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                {/* Financial Year */}
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5 block">Financial Year</label>
                  <select value={fyId} onChange={e => setFyId(e.target.value)}
                    ...
                    {financialYears.map(fy => <option key={fy._id} value={fy._id}>{fy.label}</option>)}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select value={proId} onChange={e => setProId(e.target.value)}
                    className="w-full bg-[#0d1b2a] border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-gold/50">
                    {activePros.map(p => <option key={p._id} value={p._id}>{p.name} — {p.area}</option>)}
                  </select>
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5 block">
                    Amount (₹) — <span className="normal-case text-gray-500">Total Collection = this amount</span>
                  </label>
                  <input
                ...
                <div className="flex gap-3 pt-1">
                  <button type="submit" className="flex-1 py-2.5 rounded-xl font-bold text-sm text-dark-bg transition-all"
                    style={{ background: `linear-gradient(135deg, ${moduleColor}, ${moduleColor}cc)` }}>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#0c1224]/90 backdrop-blur-md border border-rose-500/30 rounded-2xl py-4 px-6 flex items-center justify-between gap-6 z-40 shadow-2xl shadow-rose-950/20 max-w-lg w-[90vw] animate-slideUp">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-[11px] leading-tight mt-0.5">Delete selected monthly collection entries.</p>
            </div>
          </div>
